...
void GameEventHandler::updateInput(uint32 inputCode, float direction, bool isRepeat) {
    if (isRepeat)
        return;
    for (uint32 i = 0; i < inputs[inputCode].size(); i++)
        inputs[inputCode][i].second.addAmount(inputs[inputCode][i].first * direction);
}
